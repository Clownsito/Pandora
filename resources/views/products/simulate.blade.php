...
<h3>Precio manual</h3>
<input type="number" id="sale_price" placeholder="Ingrese precio">
<select id="marketplace">
<option value="">Web propia (sin comisión)</option>
@foreach(\App\Models\Marketplace::where('company_id',$product->company_id)->get()->where('commission_percent', '>', 0) as $m)
<option value="{{ $m->id }}">
...
salePriceInput.addEventListener('focus', () => { userEditing = true; });
salePriceInput.addEventListener('blur', () => { userEditing = false; });

function usePrice(p){
    if (!userEditing) {
        salePriceInput.value = p;
    }
}
